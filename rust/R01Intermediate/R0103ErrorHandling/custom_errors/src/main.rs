...
//                 f.write_str(&format!("{op} is not a valid operation. Allowed: +,-,/,*"))
//             }
//             CalculationError::InvalidOperand(op) => {
//                 f.write_str(&format!("{op} is not a valid integer in range [-128, 127]"))
//             }
//             CalculationError::DivideByZero { dividend } => f.write_str(&format!(
//                 "Can not divide by zero. Attempting to divide {dividend} by 0"
//             )),
//             CalculationError::Overflow => f.write_str("Overflow while performing the operation"),
//         }
//     }
// }
